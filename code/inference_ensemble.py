...
def load_test_dataset(dataset_dir, tokenizer, token_type, model):
    """
    test dataset을 불러온 후,
    tokenizing 합니다.
    """
    ...
    tokenized_test = tokenized_dataset(test_dataset, tokenizer, token_type, model)
...
def main(args):
    ...
    if args.ensemble:
            ...
            if "klue" in arc_name:
                ...
                if "bert-" in arc_name:
                    ...
                    test_id, test_dataset, test_label = load_test_dataset(
                        test_dataset_dir, tokenizer, folder.split(" ")[-1], "bert"
                    )
                else:
                    Tokenizer_NAME = arc_name
                    tokenizer = AutoTokenizer.from_pretrained(Tokenizer_NAME)
                    test_id, test_dataset, test_label = load_test_dataset(
                        test_dataset_dir, tokenizer, folder.split(" ")[-1], "robert"
                    )
                ...
                tokenizer = AutoTokenizer.from_pretrained(Tokenizer_NAME)
                test_id, test_dataset, test_label = load_test_dataset(
                    test_dataset_dir, tokenizer, folder.split(" ")[-1], "xlm"
                )
            else:
                Tokenizer_NAME = arc_name
                tokenizer = AutoTokenizer.from_pretrained(Tokenizer_NAME)
        ...
        # elif args.ensemble_option == "hard":
        #     mean_prob = np.mean(output_probs, axis=0)
        #     pred_answer = np.argmax(mean_prob, axis=-1)
        #     if args.use_data_helper:
        #         custom_mean_prob = np.mean(custom_output_probs, axis=0)
